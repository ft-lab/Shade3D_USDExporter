 ...
 * @param[in] shape      対象形状.
 */
void CAnimKeyframeBake::storeKeyframes (sxsdk::shape_class* shape)
{
	clear();
	if (!shape) return;
	...
	try {
		...
		if (m_keyframeData.size() <= 1) return;
		if (m_exportParam.animKeyframeMode == USD_DATA::EXPORT::ANIM_KEYFRAME_MODE::anim_keyframe_only) return;
