...
|Shade3Dのパラメータ|USDでのパラメータ|使用要素|意味|
|---|---|---|---|
|拡散反射 (値と色)|diffuseColor|RGB|色(RGB)情報|
|反射 (値)|metallic|グレイスケール|反射（映り込み）の強さ|
|透明 (値)|1.0 - opacity|グレイスケール|不透明度(opacity)の逆|
|屈折 (値)|ior|値のみ|屈折率|
|荒さ (値)|roughness|グレイスケール|ざらつきの強さ|
|発光 (値と色)|emissive|RGB|発光の強さ|
